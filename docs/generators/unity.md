...
Each [exposed parameter](../getting-started/patching.md#exposing-parameters) will automatically generate a parameter in the Unity Editor interface. These parameters can be scripted easily from elsewhere in your project.
...
[Exposed events](../getting-started/patching.md#exposing-events) are a way to generate triggers or 'bangs' into your patch. Adding one will automatically generate a button in the Unity Editor interface. These events can be scripted easily from elsewhere in your project.
...
Install [CMake Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cmake-tools) and open the `unity` folder containing the `CMakeLists.txt` in VSCode.
...
That's all that is needed when using [Audio Libs](unity.md#audio-libs).

For [Audio Plugins](unity.md#audio-plugins) the additional files from `unity/src/` will also need to be copied into the Xcode project, though ignore the C# (.cs) file.
